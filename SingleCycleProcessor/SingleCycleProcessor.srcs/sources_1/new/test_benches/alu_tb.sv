...
module alu_tb();
  ...
  logic zero_exp;
  int errors = 0;
	parameter int tests = 23; 
  
    // TESTS FORMAT: {a, b, ALUControl, result_exp, zero_exp}
    logic [196:0] testvectors [0:tests-1] = '{
                // 2_POSITIVOS	 
                {64'd3, 64'd1, 4'b0000, 64'd1, 1'b0},  // &
                {64'd1, 64'd1, 4'b0001, 64'd1, 1'b0},  // |
                {64'd1, 64'd1, 4'b0010, 64'd2, 1'b0}, // +
                {64'd2, 64'd1, 4'b0110, 64'd1, 1'b0}, // -
                {64'd1, 64'd3, 4'b0111, 64'd3, 1'b0}, // pass b

                // 2_NEGATIVOS
                {-64'd3, -64'd1, 4'b0000, -64'd3, 1'b0}, // &
                {-64'd1, -64'd1, 4'b0001, -64'd1, 1'b0}, // |
                {-64'd1, -64'd1, 4'b0010, -64'd2, 1'b0}, // +
                {-64'd2, -64'd4, 4'b0110, 64'd2, 1'b0}, // -
                {-64'd1, -64'd3, 4'b0111, -64'd3, 1'b0}, // pass b

                // 1 POSITIVO 1 NEGATIVO
                {64'd3, -64'd1, 4'b0000, 64'd3, 1'b0}, // &
                {64'd1, -64'd1, 4'b0001, -64'd1, 1'b0}, // |
                {-64'd1, 64'd1, 4'b0010, 64'd0, 1'b1}, // +
                {64'd2, -64'd1, 4'b0110, 64'd3, 1'b0}, // -
                {64'd1, -64'd3, 4'b0111, -64'd3, 1'b0}, // pass b
                ...
                // testing zero output
                {64'd2, 64'd1, 4'b0000, 64'd0, 1'b1},  // &
                {64'd0, 64'd0, 4'b0001, 64'd0, 1'b1},  // |
                {-64'd2, 64'd2, 4'b0010, 64'd0, 1'b1}, // +
                {-64'd1, -64'd1, 4'b0110, 64'd0, 1'b1}, // -
                {64'd2, 64'd0, 4'b0111, 64'd0, 1'b1}, // pass b
					 
                // Testing default value 
                {64'd2, 64'd0, 4'b0101, 64'd0, 1'b1}, // Other ALUControl value
                {64'd2, 64'd0, 4'b1000, 64'd0, 1'b1} // Other	ALUControl value				 
